...
#Loop for iterating through dataset
while 1:
	...
	#This code in this form works for postprocessing a dataset. For real time add camera implementation	    
	for img in sorted(glob.glob('/home/harrit/RCCAR_Conventional/itic_maintrack/*.png')):  
		#Reading the images using OpenCV2	
		cv_img = cv2.imread(img)
		#Applying Gaussian filtering 
		cv_img_filtered = cv2.GaussianBlur(cv_img,(5,5),0)
		...
		cropfrom_y = height/2
		croptill_y = 3*height/4
		cropheight = croptill_y - cropfrom_y
		cropfrom_x = width/4
		croptill_x = 3*width/4 
		...
		lines = cv2.HoughLinesP(image_edges, 1, np.pi/180.0, 70, np.array([]), 5, 5)
		...
		left = [[x_av1_l,y_av1_l],[x_av2_l,y_av2_l],[x_av3_l,y_av3_l],[x_av4_l,y_av4_l],[x_av5_l,y_av5_l],[x_av6_l,y_av6_l]]
		right =[[x_av1_r,y_av1_r],[x_av2_r,y_av2_r],[x_av3_r,y_av3_r],[x_av4_r,y_av4_r],[x_av5_r,y_av5_r],[x_av6_r,y_av6_r]]		
		


		#-------------------------Yuliang's trajectory implementation-------------------------#
		with open("loglanefollowing.txt", "a") as text_file:
			tjec = fol.gettrajectory(cropheight,cropwidth,left,right,160,text_file)
			angle = fol.getsteeringangle(cropheight,cropwidth,left,right,160,text_file)
			print angle
		if tjec is not None:
			endlen = len(tjec)
    			for i in range(0,endlen-1):
        			cv2.line(cv_img_cropped,(tjec[i][0],tjec[i][1]),(tjec[i+1][0],tjec[i+1][1]),(255,128,0),3)



		#Writing the processed images to a folder 	
		cv2.imwrite('/home/harrit/RCCAR_Conventional/results/' + str(j) + '.png',cv_img_cropped)   	#Drawing the detected lines over the original images	
